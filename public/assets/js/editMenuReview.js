document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.edit-review-form').forEach(function(form) {
        form.addEventListener('submit', function(e) {
            e.preventDefault();
            const reviewId = form.dataset.reviewId;
            const formData = new FormData(form);

            fetch('/menu_reviews/edit_review', {
                method: 'POST',
                body: formData,
            })
                .then(res => res.json())
    ...
    document.querySelectorAll('.cancel-edit').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            ...
            const reviewDiv = document.getElementById('review-' + reviewId);
            if (reviewDiv) {
                reviewDiv.querySelector('.edit-review-form').style.display = 'none';
                reviewDiv.querySelector('.review-content').style.display = '';
            }
        });
    });
    ...
    document.querySelectorAll('.edit-btn').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            ...
            if (reviewDiv) {
                ...
                reviewDiv.querySelector('.edit-review-form').style.display = '';
            }
        });
    });
});
